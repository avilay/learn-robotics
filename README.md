...
The ports are needed for all the APIs and CLIs. To make my life easier and not have to find which USB ports the robots and cameras are connected to everytime I disconnect and reconnect my laptop to my USB hubs, I have written `setenv.py` script that will set the `YANTRA_*` environment variables by automatically detecting the relevant USB ports. I need to follow this sequence:
...
Once on a CUDA-enabled VM download the Docker image:
